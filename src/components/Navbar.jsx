...
const Navbar = ({ currentPage, onPageChange }) => {
  ...
  // Get active link from URL on component mount (fallback)
  useEffect(() => {
    if (!currentPage) {
  ...
  const handleNavClick = (item) => {
    ...
    // Jika onPageChange prop tersedia, gunakan untuk SPA
    if (onPageChange && typeof onPageChange === 'function') {
      onPageChange(item.id);
      // Optional: Update URL tanpa reload halaman
      window.history.pushState({}, '', item.path);
    } else {
      // Fallback: gunakan navigasi tradisional
